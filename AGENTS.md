...
- 模块结构：模块选择页当前包含“中级经济师－人力资源”“中级经济师－经济学”“英语”“PMP－项目管理专业人士”四个大类。人力资源模块继续使用原 `/study`、考试、错题和成绩路由及现有数据；经济学、英语和 PMP 拥有独立模块首页，不得复用或展示人力资源数据。PMP 首页为 `/study/modules/pmp`，导学、考试与成绩分别使用 `/study/modules/pmp/materials/:materialId`、`/study/modules/pmp/exams/:examId`、`/study/modules/pmp/results/:resultId`。英语模块固定以 Listening/听、Speaking/说、Reading/读、Writing/写四板块扩展；当前 Listening/听已开放，目录路由为 `/study/modules/english/listening`，场景练习路由为 `/study/modules/english/listening/:sceneId`，“每日听闻”路由为 `/study/modules/english/listening/daily/:storyId`。听力模块只训练“听”，采用盲听、作答、提交后精析、正常或慢速复测的顺序，不得加入录音、麦克风、跟读或口语评分；发音参考保持折叠，并使用真人录制的完整记忆词组帮助辨音，不得让 Web Speech API 朗读孤立单词或把机器音作为兜底。场景对话、每日听闻与发音参考必须播放已同步到私有 MinIO 的真人 MP3；真人音频缺失、存储异常或播放失败时必须向用户显示明确错误，不得静默回退。新增学习类型时应扩展模块目录，而不是混入现有学科。模块选择页保持紧凑、可扩展的响应式网格：手机端使用双列卡片，桌面端根据可用宽度自动排布，不为最后一张卡片设置跨列特例。
...
- 课程权限：`learning_modules` 是课程目录，`user_module_access` 是账号可见课程的唯一事实源。前端课程选择页必须按当前账号的 `moduleIds` 过滤，所有课程专属 API 和前端深层路由也必须独立校验权限，不能只依赖隐藏卡片。版本 10 迁移时 PMP 只自动分配给用户名为 `doudou` 的账号；命令行新建其他账号时默认也排除 PMP，只能通过显式课程分配改变。管理员页面仅限 `is_admin = 1` 且 `is_active = 1` 的账号进入；系统必须始终保留至少一个启用中的管理员，管理员不能停用、降权或删除自己。
...
- 管理员课后作业：`/study/admin/homework` 是独立于“学习资料”的管理员专属题库，只允许 `is_admin = 1` 且启用中的账号查看。当前固定收录“精讲班－殷巧玲”第 1—19 章、共 403 道在线题；点击章节必须进入逐题作答页面，不能再把 PDF 阅读器作为默认入口。每次作答写入独立课后题记录，答错汇入现有错题本，但不生成考试成绩、分数或考试记录。原始 PDF 只作为私有来源存档和兼容接口保留，目录、题目、图片和 PDF 均不得向普通用户开放。
...
- 校验并同步管理员课后作业 PDF 与题面图片：`npm run homework:sync-admin -- --source-dir <含 19 份 PDF 和 question-assets 目录的课后作业目录>`
- 从管理员课后作业 PDF 重建在线题库：`python3 scripts/build-admin-homework-questions.py --source-dir '<19 份 PDF 所在目录>' --output server/admin-homework-questions.json --asset-output-dir '<同一目录>/question-assets'`
...
- 命令行创建或重置用户：`npm run user:upsert -- --username <用户名> --display-name <昵称> [--modules human-resources,economics,english,pmp|none] [--admin true|false]`
...
- 数据库结构当前为版本 11：`users.is_admin` 与 `users.is_active` 保存管理和启停状态，`learning_modules` 保存课程目录，`user_module_access` 保存课程分配，`exam_modules` 保存试卷与课程的唯一归属，`admin_audit_log` 保存网页及命令行账号管理记录，`daily_listening_attempts` 保存按用户隔离的每日听闻提交记录，`homework_question_attempts` 保存不计分的管理员课后题逐次作答。所有网页管理写操作必须写入操作者、目标账号、请求 ID、变更摘要和时间，不得记录明文密码。
- PMP 内容以 `server/pmp-content.mjs` 为单一事实源，当前固定包含 6 篇原创导学、3 套每套 12 题且 20 分钟的原创情境短卷，以及 PMI 官方认证页、2026 考试内容纲要、备考页与认证资源页链接。不得复制或导入来源不明的“真题”；PMI 公开样题只提供官方入口。考试信息当前按 2026 年 7 月新版维护，70 分仅是站内训练目标，不得表述为 PMI 固定合格线。PMP 接口必须按 `exam_modules.module_id = 'pmp'` 隔离试卷与成绩，提交前不得返回正确选项或解析。PMP 原创导学不受通用 PDF 学习资料开关影响。
...
- 管理员课后作业清单及题面图片清单以 `server/admin-homework-content.mjs` 为单一事实源，结构化题库以 `server/admin-homework-questions.json` 为单一事实源；必须保持第 1—19 章连续、各章题量固定且总计 403 题。`scripts/build-admin-homework-questions.py` 必须从原始 PDF 严格核对章节、题号、答案、解析和两张案例图并在歧义时失败，不得静默漏题或虚构内容。题目接口提交前不得返回正确选项或解析；作答只写入 `homework_question_attempts`，不得写入 `attempts`，答错题通过现有错题与重练接口展示，“已重学”规则保持不变。PDF 与两张题面图片保存在私有 MinIO，同步命令必须校验精确字节数和 SHA-256 并在上传后回读对象大小；网页响应不得返回对象键、源文件名或哈希。PDF 兼容接口 `/api/admin/homework/:chapterId/file` 与图片接口 `/api/admin/homework/:chapterId/assets/:assetId` 都必须管理员鉴权并核对对象大小，普通登录用户和未登录请求分别返回 403 与 401。该题库不受通用学习资料开关影响，也不得借此恢复 `/study/materials` 或 `/api/materials`。
...
- 首次启用或更新管理员课后作业时，将 19 份原始 PDF 和生成器产出的 `question-assets` 目录放入只读挂载的 `/app/imports/admin-homework-2026`，执行 `docker compose run --rm app npm run homework:sync-admin -- --source-dir /app/imports/admin-homework-2026`；同步完成后必须核对 19 份 PDF 和 2 张题面图片的私有对象、在线题目总数 403、目录及答题接口的管理员权限、普通用户 403、未登录 401、题面图片访问，以及 PDF Range 请求的 206 响应。部署过程不得修改 `VITE_MATERIALS_ENABLED=false` 与 `MATERIALS_ENABLED=false`。
- 部署数据库迁移前必须创建 SQLite 备份；迁移到版本 8 时，所有已有账号默认获得当前三门课程，按 `created_at`、`id` 排序最早的已有账号成为首位管理员，以保持旧账号可继续使用；迁移到版本 9 时新增 `daily_listening_attempts`，不改动已有账号、考试、错题或场景听力记录；迁移到版本 10 时新增 PMP 课程、`exam_modules`、3 套 PMP 短卷，把原有试卷归属到 `human-resources`，并且只给用户名为 `doudou` 的已有账号追加 PMP 权限；迁移到版本 11 时新增 `homework_question_attempts` 并写入 19 章、403 道管理员课后题，不改动既有考试成绩和错题记录。迁移后需核对用户数量、管理员数量、课程分配数量、试卷归属与题量、账号成绩数量、课后题作答数量、听力记录数量和未归属旧记录数量。
